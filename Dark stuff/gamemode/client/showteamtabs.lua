...
function GM:JobsTab()
	...
	function hordiv:Update()
		...
		for k,v in ipairs(RPExtraTeams) do -- standard jobs
			...
			--		nodude = false
			--	end
				--if v.customCheck and not LocalPlayer():IsUserGroup("vip") then
					--nodude = false
				--end
				if v.isVIP == 1 then
					nodude = false
...
function GM:VIPJobs() -- BECAUSE JOBS LISTS GET CROWDED SO LETS FIX THAT HUE
	local hordiv = vgui.Create("DHorizontalDivider")
	hordiv:SetLeftWidth(390)
	function hordiv.m_DragBar:OnMousePressed() end
	hordiv.m_DragBar:SetCursor("none")
	...
	function hordiv:Update()
		...
		for k,v in ipairs(RPExtraTeams) do
			...
			--	if v.customCheck and LocalPlayer():IsUserGroup("vip") then
			--		nodude = true
			--	end
				if v.isVIP == 1 then
					nodude = true
...
function GM:AdminJobs() -- BECAUSE JOBS LISTS GET CROWDED SO LETS FIX THAT HUE
	local hordiv = vgui.Create("DHorizontalDivider")
	hordiv:SetLeftWidth(390)
	function hordiv.m_DragBar:OnMousePressed() end
	hordiv.m_DragBar:SetCursor("none")
...
function GM:LyraJobs() -- BECAUSE JOBS LISTS GET CROWDED SO LETS FIX THAT HUE
	local hordiv = vgui.Create("DHorizontalDivider")
	hordiv:SetLeftWidth(390)
	function hordiv.m_DragBar:OnMousePressed() end
	hordiv.m_DragBar:SetCursor("none")
...
function GM:EntitiesTab()
	...
	EntitiesPanel:EnableVerticalScrollbar( true )
		function EntitiesPanel:Update()
				...
				AmmPanel:EnableVerticalScrollbar(true)
					...
					for k,v in pairs(GAMEMODE.AmmoTypes) do
						if not v.customCheck or v.customCheck(LocalPlayer()) then
							AddAmmoIcon(v.model, string.format(LANGUAGE.buy_a, v.name, CUR .. v.price), "/buyammo " .. v.ammoType)
						end
					end
					
					AmmCat:SetContents(AmmPanel)
			AmmCat:SetSkin("DarkRP")
			self:AddItem(AmmCat)
			if LocalPlayer():Team() == TEAM_GUN or LocalPlayer():Team() == TEAM_PGUN or LocalPlayer():Team() == TEAM_ADMIN or LocalPlayer():Team() == TEAM_EXPLODE or LocalPlayer():Team() == TEAM_BMD or LocalPlayer():Team() == TEAM_MEDIC or LocalPlayer():Team() == TEAM_UMEDIC or LocalPlayer():Team() == TEAM_SHINING then
			...
			if LocalPlayer():Team() == TEAM_GUN or LocalPlayer():Team() == TEAM_PGUN or LocalPlayer():Team() == TEAM_ADMIN or LocalPlayer():Team() == TEAM_EXPLODE or LocalPlayer():Team() == TEAM_DRUG or LocalPlayer():Team() == TEAM_EXDRUG or LocalPlayer():Team() == TEAM_BMD or LocalPlayer():Team() == TEAM_MEDIC or LocalPlayer():Team() == TEAM_UMEDIC or LocalPlayer():Team() == TEAM_SHINING then
				...
				ShipPanel:EnableVerticalScrollbar(true)
					...
					for k,v in pairs(CustomShipments) do
						if not v.noship and table.HasValue(v.allowed, LocalPlayer():Team())
							and (not v.customCheck or (v.customCheck and v.customCheck(LocalPlayer()))) then
							AddShipIcon(v.model, string.format(LANGUAGE.buy_a, "a "..v.name .." shipment", CUR .. tostring(v.price)), "/buyshipment "..v.name)
						end
					end
					ShipCat:SetContents(ShipPanel)
			ShipCat:SetSkin("DarkRP")
			self:AddItem(ShipCat)
			end
			
			local EntCat = vgui.Create("DCollapsibleCategory")
			EntCat:SetLabel("Printers")
				...
				EntPanel:EnableVerticalScrollbar(true)
					...
					for k,v in pairs(DarkRPEntities) do
						if not v.allowed or (type(v.allowed) == "table" and table.HasValue(v.allowed, LocalPlayer():Team()))
							and (not v.customCheck or (v.customCheck and v.customCheck(LocalPlayer()))) then
							if v.isVip == 0 or v.isVip == nil and (v.cookmenu == 0 or v.cookmenu == nil)  then
							local cmdname = string.gsub(v.ent, " ", "_")

							AddEntIcon(v.model, "Buy a " .. v.name .." " .. CUR .. v.price, v.cmd)
							end
							else if v.isVip == 1 or v.name == "Microwave" then
							end
						end
					end
			...
			local VipCat = vgui.Create("DCollapsibleCategory")
			VipCat:SetLabel("VIP Printers")
				local VipPanel = vgui.Create("DPanelList")
				VipPanel:SetSize(470, 200)
				VipPanel:SetAutoSize(true)
				VipPanel:SetSpacing(1)
				VipPanel:EnableHorizontal(true)
				VipPanel:EnableVerticalScrollbar(true)
					local function AddVipIcon(Model, description, command)
						local icon = vgui.Create("SpawnIcon")
						icon:InvalidateLayout( true )
						icon:SetModel(Model)
						icon:SetSize(64, 64)
					...
					for k,v in pairs(DarkRPEntities) do
						if not v.allowed or (type(v.allowed) == "table" and table.HasValue(v.allowed, LocalPlayer():Team()))
							...
							AddVipIcon(v.model, "Buy a " .. v.name .." " .. CUR .. v.price, v.cmd)
							end
							else if v.isVip == 0 or v.isVip == nil then
							end
						end
					end
...
--					if FoodItems and (GAMEMODE.Config.foodspawn or LocalPlayer():Team() == TEAM_COOK) and (GAMEMODE.Config.hungermod or LocalPlayer():Team() == TEAM_COOK) then
			...
			VipCat:SetContents(VipPanel)
			VipCat:SetSkin("DarkRP")
			self:AddItem(VipCat)
			
			if LocalPlayer():Team() == TEAM_COOK then
			local CookCat = vgui.Create("DCollapsibleCategory")
			CookCat:SetLabel("Cook menu")
				local CookPanel = vgui.Create("DPanelList")
				CookPanel:SetSize(470, 200)
				CookPanel:SetAutoSize(true)
				CookPanel:SetSpacing(1)
				CookPanel:EnableHorizontal(true)
				CookPanel:EnableVerticalScrollbar(true)
					local function AddCookIcon(Model, description, command)
						local icon = vgui.Create("SpawnIcon")
						icon:InvalidateLayout( true )
						icon:SetModel(Model)
						icon:SetSize(64, 64)
						icon:SetToolTip(description)
						icon.DoClick = function() LocalPlayer():ConCommand("darkrp "..command) end
						CookPanel:AddItem(icon)
					end

					for k,v in pairs(DarkRPEntities) do
						if not v.allowed or (type(v.allowed) == "table" and table.HasValue(v.allowed, LocalPlayer():Team()))
							and (not v.customCheck or (v.customCheck and v.customCheck(LocalPlayer()))) then
							if v.name == "Microwave" then
							local cmdname = string.gsub(v.ent, " ", "_")

							AddCookIcon(v.model, "Buy a " .. v.name .." " .. CUR .. v.price, v.cmd)
							end
							else if v.name != "Microwave" then
							end
						end
					end
					...
					if FoodItems and (GAMEMODE.Config.foodspawn or LocalPlayer():Team() == TEAM_COOK) and (GAMEMODE.Config.hungermod or LocalPlayer():Team() == TEAM_COOK) then
						for k,v in pairs(FoodItems) do
							AddCookIcon(v.model, string.format(LANGUAGE.buy_a, "a "..k, "$15"), "/buyfood "..k)
						end
					end
					
			CookCat:SetContents(CookPanel)
			CookCat:SetSkin("DarkRP")
			self:AddItem(CookCat)
			end
